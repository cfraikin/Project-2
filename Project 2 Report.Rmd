...
# Get totals per day through summation
confirmed_sum<-confirmed_ds%>%filter(Province_State=="California")%>%select(-(1:11))%>%summarise_all(sum)
death_sum<-deaths_ds%>%filter(Province_State=="California")%>%select(-(1:12))%>%summarise_all(sum)
...
# Create Plot
ggplot(data_long, aes(x=Dates, y=SumCases, color=dataset)) + 
  scale_color_discrete(breaks=c("Confirmed", "Deaths")) +
  ...
  labs(fill = "California Total")


#need to change legend name 
last_date <- names(confirmed_ds)[ncol(confirmed_ds)]
top_confirmed_cities<-confirmed_ds%>%filter(Province_State=="California")%>%group_by(Admin2)%>%select(-(1:11))%>%summarise_all(sum)%>%arrange(-X5.14.20) #having a hard time making this the last column with last_date
dat<-top_confirmed_cities[1:3,]
data_long <- gather(dat, Dates, SumCases, -Admin2, factor_key=TRUE)

# Create Plot
ggplot(data_long, aes(x=Dates, y=SumCases, color=Admin2)) + 
  scale_color_discrete(breaks=c("Los Angeles", "Riverside", "San Diego")) +
  geom_point() + ggtitle("California Trajectory for COVID-19") + 
  scale_x_discrete(breaks=c("X2.1.20","X3.1.20","X4.1.20", "X5.1.20"),
                   labels=c("Feb", "Mar", "Apr", "May")) +
  labs(y= "Cases", x = "Date") +
  scale_y_continuous(breaks=pretty_breaks(n=5), 
